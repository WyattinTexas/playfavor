...
RIDER DONE — card "TASK FROM THE GO MACHINE 9/2 22:05" (the 0720 rider; acct G, corkscreweducation@gmail.com, 22:52 → 22:58 ET, session 11% → 12%): Wyatt RULED §4 Q1 — the title theme comes back FROM WHERE IT PAUSED. That is what `1214dd9` shipped (re-read on HEAD and re-proven on the served bytes, §4.1), so nothing was built: this doc only, no `?v=` bump, game bytes untouched. Still owed: Wyatt's ear-check on the phone (§4.3).
...
1. **RULED — Q1: from where it paused.** Wyatt, Wednesday 9/2/2026 ~9:38 pm ET, verbatim: "2) Yes where its paused." (his "1)" answered GVT's question and rode the gvt lane). From-where is the law, not a shipped assumption: a hide/return resumes the SAME pass from the held second, never the top. That is exactly what `1214dd9` shipped — re-confirmed by the 0720 rider on 9/2 22:57 ET: HEAD's `themeResume()` calls `play()` with no seek (the only rewinds in the pocket code are the deal's stop and a fade already dying, both by design), the served js/sfx.js md5 `4ca53d31` == HEAD under the live stamp `20260902215117`, and one more headless hide/return leg on the live bytes ran 48/48 with zero console errors (hidden froze the pass at 0.67 s, the return played on 0.67 → 0.92 s, a vetoed return resumed 2.37 → 2.68 s on the next gesture, `pageshow` released from the held second). The from-the-top one-liner (`theme.currentTime = 0` before `play()` in `themeResume()`) stays unbuilt; if Wyatt ever wants it, it is still that one line.
...
RIDER · QUIET WHEN PUT AWAY · Q1 RULED from-where (Wyatt 9/2 ~21:38 ET) · nothing built, this doc only, no ?v= bump · live stamp still 20260902215117, served js/sfx.js md5 still 4ca53d31 == HEAD · live headless leg on playfavor.net 22:57 ET 48/48, zero console errors · pushed 22:58 ET; the Pages run (completed success) is recorded in the memory favor-quiet-when-put-away-0902.
