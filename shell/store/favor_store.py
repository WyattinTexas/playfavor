...
NOTE ON THE 2.1(b) REJECTION OF THIS SUBMISSION (Aug 24): the four packs were in the binary (StoreKit 2, Product.products(for:) over com.corkscrewgames.favor.stars.s/.m/.l/.xl) but the Royal Mint sheet only appeared once StoreKit had returned priced products, and a single empty product answer at launch hid the sheet for the rest of the session. Corrected Aug 26: the "Purchase Stars" door is always present on this build, the product request is retried automatically and again whenever the store is opened, and each pack shows its storefront price the moment StoreKit answers (an UNAVAILABLE label appears only while the storefront has not answered). The game's interface is served from playfavor.net, so build 22 already runs the corrected store — no new binary was required.
